...
- **GitHub Environments:** the `deploy` job uses `environment: dev` or `prod`. Create those environments in **GitHub → Settings → Environments** if you use manual dispatch.
...
| ---- | ------------------------- |
| Job has **no** GitHub Environment (e.g. `build-and-push` in `azure-deploy.yml`) | `repo:OWNER/REPO:ref:refs/heads/main` |
| Job uses **GitHub Environment** `prod` (e.g. `deploy` in `azure-deploy.yml`) | `repo:OWNER/REPO:environment:prod` |
| Same for `dev` | `repo:OWNER/REPO:environment:dev` |

Replace `OWNER/REPO` with your real path (case-sensitive). If `build-and-push` passes but `deploy` fails with an auth error, add the **environment** subjects for `dev` and `prod`.

...
| `quack_db/config.py` | `pydantic-settings` env (`DATABASE_URL`, Chroma, Entra JWT, Azure OpenAI, …) |
...
| `quack_db/db/models.py` | SQLAlchemy models: `User`, `Session`, `Message` |
| `quack_db/db/session.py` | Engine + `get_db()` dependency helper |
| `quack_db/db/migrations/` | Alembic revisions — also run on API startup |
| `auth/entra_jwt.py` | Entra access-token verification (JWKS, RS256) |
...
| `ingest/cli.py` | Console script **`quack-ingest`** — POSTs chunks to `/collections/{name}/add` with Bearer |
